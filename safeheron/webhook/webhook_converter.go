...
func (c *WebhookConverter) Convert(d WebHook, webHookRsaPrivateKey *rsa.PrivateKey) (string, error) {
	...
	if d.RsaType == utils.ECB_OAEP {
		plaintext, _ = utils.DecryptWithOAEP(d.Key, webHookRsaPrivateKey)
	} else {
		plaintext, _ = utils.DecryptWithRSA(d.Key, webHookRsaPrivateKey)
	}
