...
func (c Client) execute(request any, endpoint string) ([]byte, error) {
	...
	signature, err := utils.SignParamsWithRSA(serializeParams(params), c.ApiPrivateKey)
	if err != nil {
		return nil, err
	}
	...
	if utils.ECB_OAEP == responseStruct.RsaType {
		plaintext, _ = utils.DecryptWithOAEP(responseStruct.Key, c.ApiPrivateKey)
	} else {
		plaintext, _ = utils.DecryptWithRSA(responseStruct.Key, c.ApiPrivateKey)
	}
